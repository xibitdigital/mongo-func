...
function getConnection(connectionObject) {
  return new Promise(function (resolve, reject) {
    ...
    if (R.keys(hasConnection).length !== 0) {
      ...
      _mongodb.MongoClient.connect(connectionString, options, function (err, client) {
        // extract db from connection string
        console.log('connectionString', connectionString);
        var database = connectionString.match(/.+:\d+\/([a-z-_.]+)/i)[1];
